...
Raone.useCmd({pattern: 'insert ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN, dontAddCommandList: true}, (async (message, match) => {

    if (match[1] == '') return await message.client.sendMessage(message.jid,Lang.NEED_URL + '.install https://gist.github.com/RAVANA-SL/4232b1c8c4734e1f06c3d991149c6fbd', MessageType.text)
    try {
    ...
    if (response.statusCode == 200) {
        ...
        try {
            ...
            fs.unlinkSync('/root/slRavana/RavanaBot/Cmd' + plugin_name + '.js')
        ...
        if (response.body.includes('../../DataBase/notes') || response.body.includes('../../DataBase/plugin') || response.body.includes('../../DataBase/greetings') || response.body.includes('../../DataBase/filters')) DEG.level = DEG.level + 33
...
Raone.useCmd({pattern: 'plugin$', fromMe: true, dontAddCommandList: true, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
    ...
    if (plugins.length < 1) {
        ...
        plugins.map(
            (plugin) => {
                let vf = plugin.dataValues.url.includes('RAVANA-SL') ? msg : unmsg
...
Raone.useCmd({pattern: 'remove(?: |$)(.*)', fromMe: true, dontAddCommandList: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
